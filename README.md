...
| --- | --- | --- |
| Acceptance length (GSM8K, greedy) | 1.00 | <<<MEASURED:acceptance>>> |
| Decode tok/s (RTX 3090, mcs 320) | <<<MEASURED:decode_off>>> | **<<<MEASURED:decode_on>>>** |
| 256-token TTFT | <<<MEASURED:ttft>>> s | |

At 200k-token depth: prefill <<<MEASURED:prefill_tps>>> tok/s, decode
<<<MEASURED:decode_200k>>> tok/s. Full numbers and raw JSON: `notes/PERF.md`,
`metrics/`.
...
| artifact | size |
| --- | --- |
| EXL3 2.50 bpw (vision 6, MTP 4, n-gram table 3) | <<<MEASURED:weights_gib>>> GiB |
...
| state | usage |
| --- | --- |
| loaded and idle (torch) | <<<MEASURED:vram_idle>>> GB |
| peak during long-context requests | <<<MEASURED:vram_peak>>> GB (≥ 1.5 GiB margin kept) |

Budget math behind those numbers:

- routed experts: <<<MEASURED:gpu_experts>>> of 512 per layer reside in VRAM; the tail
...
- CPU expert tail RAM: ≈ <<<MEASURED:ram_experts>>> GiB (+ ~2 GiB ring buffers); 6-core
  Zen 5 host, `-mct 6`
...
| MTP acceptance | `scripts/acceptance_check.py` | <<<MEASURED:gate_mtp>>> |
| Vision | examples/multimodal.py | <<<MEASURED:gate_vision>>> |
| 262k load + 200k prefill | `scripts/long_context_check.py` | <<<MEASURED:gate_longctx>>> |
| Multi-needle NIAH 262,080 | `scripts/niah_multikey.py` | <<<MEASURED:gate_niah>>> |
| Q200v2 text-180 (frozen kit) | `scripts/run_q200v2.sh` | <<<MEASURED:gate_q200v2>>> |
